//TODO Part of the code is leading to some negative results
...
void main_calc_steps(int n, int age_classes, double *S, double *E1, double *E2, double *E3, double *I, double *R, double *D,
                        ...
                        double *popmat, double *O, double *distmat, double tranport_mod_val, double *Sij,
        ...
        // Additionally calculate the population size at each location
        double totalpop = 0;
        ...
        // Pre-calculate the Sij matrix for use in our calculation of the steps for the rad model
        // We sum the populations sizes for each circle of distances between the original and final locations
        time0 = omp_get_wtime();
        #pragma omp parallel for
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                int distval = distmat[j * n + i];
                double val = 0;
                if(i != j) {
                    for(int k = 0; k < n; k++) {
                        if(distval >= distmat[k * n + i] && j != k) {
                            val += popmat[k];
                        }
                    }
                }
        ...
        // Calculate the leaving rates
        double popinv = 1 / totalpop;
        #pragma omp parallel for
        for(int i = 0; i < n; i++) {
            // Caclulate the IJ terms but "swap" due to difference in Rad model and metapop model specs
            double inv = 1 / popmat[i];
            double denom = 0;
            for(int k = 0; k < n; k++) {
                if (k != i) {
                    denom += popmat[k] * (Sij[i * n + k] - popinv);
                }
            }
            denom = 1 / denom;
            for(int j = 0; j < n; j++) {
                int iter = j * n + i;
                if(i != j) {
                    l[iter] = O[i] * inv * denom * popmat[j] * (Sij[i * n + j] - popinv);
                } else {
                    l[iter] = 0;
                }
            }
        ...
        for(int k = 1; k < age_classes; k++) {
            ...
            for(int i = 0; i<n; i++) {
                for(int j = 0; j < n; j++) {
                    ...
                    I_knext[iter] = 3.0 * sigma[i] * E3[iter] - gamma[i] * I[iter] + tranport_mod_val * l_prime[iter] * I[j * n + j] - r_prime_I[iter] - mu[iter] * I[iter] - aging[k] * I[iter] - kappa[k * n + i] * I[iter];
...
int main(int argc, char** argv) {
   ...
   // Check there are 4 arguments to our program
    if(argc != 4) {
    ...
    // This section could be updated as current values are independent of location and age
    ...
    if(!test) {
        ...
        fprintf(f, "%g, 0, %g, %g, %g, %g, %g, %g\n", 0.0,  S[0], E1[0], E2[0], E3[0], I[0], R[0]);
        fprintf(f, "%g, 1, %g, %g, %g, %g, %g, %g\n", 0.0,  S[1], E1[1], E2[1], E3[1], I[1], R[1]);
        fprintf(f, "%g, 2, %g, %g, %g, %g, %g, %g\n", 0.0,  S[2], E1[2], E2[2], E3[2], I[2], R[2]);
    }    
    // printf("%g, 0, %g, %g, %g, %g, %g, %g\n", 0.0,  S[315], E1[315], E2[315], E3[315], I[315], R[315]);
    // printf("%g, 1, %g, %g, %g, %g, %g, %g\n", 0.0,  S[316], E1[316], E2[316], E3[316], I[316], R[316]);
    // printf("%g, 2, %g, %g, %g, %g, %g, %g\n", 0.0,  S[317], E1[317], E2[317], E3[317], I[317], R[317]);
    ...
    //TODO replace hardcoded times to print regional data
    int print2times[4] = {80, 200, 324, INFINITY};

    // Initialise variable to track when to print regional data
    int *print2_prime;
    ...
    while(t_prime <= t_final - 1) {
        ...
        main_calc_steps(n, age_classes, S, E1, E2, E3, I, R, D,
                        ...
                        popmat, O, distmat, transport_mod_val, Sij,
                        half_dt, 0, debug);
        ...
        if (print) {
            ...
            // printf("%g, 0, %g, %g, %g, %g, %g, %g\n", t_prime,  S_next[315], E1_next[315], E2_next[315], E3_next[315], I_next[315], R_next[315]);
            // printf("%g, 1, %g, %g, %g, %g, %g, %g\n", t_prime,  S_next[316], E1_next[316], E2_next[316], E3_next[316], I_next[316], R_next[316]);
            // printf("%g, 2, %g, %g, %g, %g, %g, %g\n", t_prime,  S_next[317], E1_next[317], E2_next[317], E3_next[317], I_next[317], R_next[317]);
    
            if(debug) printf("%g,  %g, %g, %g, %g\n", t_prime,  Sprint, Eprint, Iprint, Rprint);
        }

        // Print regional results at requested times
        if(print2) {
