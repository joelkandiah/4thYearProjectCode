...
void main_calc_steps(int n, int age_classes, double *S, double *E1, double *E2, double *E3, double *I, double *R, double *D,
                        ...
                        double *popmat, double alpha, double gravbeta, double *O, double *distmat, double tranport_mod_val,
        ...
        for(int i = 0; i < n; i++) {
            ...
            popmat[i] = pow(tmp_value_N, alpha);
        }
        ...
        // Caculate l for each grid term using the gravity model
        #pragma omp parallel for
        for(int i = 0; i < n; i++) {
            ...
            for(int k = 0; k < n; k++) {
                if(k != i) {
                    denom += popmat[k] * pow(distmat[i * n + k], -1.0 * gravbeta);
                }
            }
            denom = 1 / denom;
            // Caclulate the IJ terms but "swap" due to difference in grav model and metapop model specs
            for(int j = 0; j < n; j++) {
                int iter = j * n + i;
                if(i != j) {
                    l[iter] = O[i] * tmpinv * popmat[j] * pow(distmat[i * n + j], -1.0 * gravbeta) * denom;
                } else {
                    l[iter] = 0;
                }
            }
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                ...
                I_knext[iter] = 3.0 * sigma[i] * E3[iter] - gamma[i] * I[iter] + tranport_mod_val * l_prime[iter] * I[j * n + j] - r_prime_I[iter] - (mu[iter] + aging[0] + kappa[i]) * I[iter];
                ...
                if(D_knext[iter] < 0) printf("D_knext[iter] = %g %d %d\n", D_next[iter], i , j);
            }
        }
        ...
        for(int k = 1; k < age_classes; k++) {
            ...
            for(int i = 0; i<n; i++) {
                for(int j = 0; j < n; j++) {
                    ...
                    I_knext[iter] = 3.0 * sigma[i] * E3[iter] - gamma[i] * I[iter] + tranport_mod_val * l_prime[iter] * I[j * n + j] - r_prime_I[iter] - (mu[iter] + kappa[i] + aging[k]) * I[iter];
...
int main(int argc, char** argv) {
   ...
   // Check there are 6 arguments to our program
    if(argc != 6) {
    ...
    double alpha, gravbeta;
    int infloc;
    sscanf(argv[1],"%lf",&alpha);
    sscanf(argv[2],"%lf",&gravbeta);
    sscanf(argv[3],"%d",&infloc);
    ...
    //! Hardcoding should be removed if possible
    int print2times[4] = {80, 200, 324, INFINITY};

    // Create variables that check whether to print data for each region
    int *print2_prime;
    ...
    while(t_prime <= t_final - 1) {
        ...
        // Check whether to print all regions
        ...
        main_calc_steps(n, age_classes, S, E1, E2, E3, I, R, D,
                        ...
                        popmat, alpha, gravbeta, O, distmat, transport_mod_val,
                        half_dt, 0, debug);
        ...
        for(int i = 0; i<n; i++) {
            for(int j = 0; j<n; j++) {
                ...
                //if(i == 1 && j == 2) printf("iter: %d %d \nsixth: %g  \nD: %g  \ndt_prime: %g \nD_k1: %g \nD_k2: %g \nD_k3: %g \nD_k4: %g \nD_next: %g\n", i, j, sixth, D[iter], dt_prime, D_k1[iter], D_k2[iter], D_k3[iter], D_k4[iter], D_next[iter]);
            }
        }
        ...
        // Print regional results if true
        if(print2) {
